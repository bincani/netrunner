...
export function DeckViewSwitcher({
  current,
  decks,
}: {
  ...
  const others = decks.filter((deck) => deck.id !== current.id)
  const filtered = trimmedQuery === '' ? others : others.filter((deck) => deck.name.toLowerCase().includes(trimmedQuery))
  const visible = filtered.slice(0, VISIBLE_LIMIT)
  const hiddenCount = filtered.length - visible.length

  function handleSelect(deckId: number) {
    setIsOpen(false)
    setQuery('')
