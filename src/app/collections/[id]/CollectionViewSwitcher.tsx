...
export function CollectionViewSwitcher({
  ...
  // Purely navigational — unlike the Dashboard's CollectionSwitcher, picking
  // a collection here does not change which one is the global default. That
  // stays a separate, explicit action (the Set as Default button), so
  // browsing to view another collection never silently changes it.
    ...
    <div ref={containerRef} className="relative">
      ...
      {isOpen && (
        ...
        >
            ...
            {visible.map((collection) => (
              <li key={collection.id}>
                <button
                  ...
                  className="w-full cursor-pointer rounded px-2 py-1 text-left text-sm hover:bg-surface-hover"
                >
                  {collection.name}
                </button>
              </li>
          ...
          {hiddenCount > 0 && <p className="mt-1 px-2 text-xs text-faint">{hiddenCount} more — refine your search</p>}
        </div>
      )}
    </div>
  )
