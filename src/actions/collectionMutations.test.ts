...
describe('collection action wiring', () => {
  ...
  it('addToCollectionMutation delegates to incrementOwned, not setOwned', async () => {
    ...
    const result = await addToCollectionMutation(prisma, 1, 1, '01007', 2)

    expect(collectionLib.incrementOwned).toHaveBeenCalledWith(prisma, 1, 1, '01007', 2)
  ...
  it('updateCollectionQuantityMutation delegates to setOwned, not incrementOwned', async () => {
    ...
    const result = await updateCollectionQuantityMutation(prisma, 1, 1, '01007', 1)

    expect(collectionLib.setOwned).toHaveBeenCalledWith(prisma, 1, 1, '01007', 1)
